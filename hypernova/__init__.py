...
class Renderer(object):
    ...
    def prepare_request(self, jobs):
        ...
        should_send_request = self.plugin_reduce(
            "should_send_request", lambda plugin, obj: obj and plugin(jobs_hash), True
        )
        return {"should_send_request": should_send_request, "jobs_hash": jobs_hash}
    ...
    def render(self, data):
        ...
        if request_data.get("should_send_request"):
            self.plugin_reduce("will_send_request", lambda plugin, _: plugin(jobs_hash))
            try:
                response = requests.post(
                    self.url, json=jobs_hash, headers=self.headers, timeout=self.timeout
                )
                if response.ok:
        ...
        for name, body in results.items():
            ...
            if error:
                ...
                self.plugin_reduce("on_error", lambda plugin, body: plugin(error, body))
            else:
                successful_jobs.append(body)

        self.plugin_reduce("on_success", lambda plugin, _: plugin(successful_jobs))

        if self.plugins:
            results = self.plugin_reduce(
                "after_response", lambda plugin, obj: plugin(obj, results), results
            )
